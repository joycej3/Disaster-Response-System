...
class LoadBalManager:
    def __init__(self, server_type, authed_session):
        self.ip = ""
    ...
    def register(self):
        if self.ip:
            try:
                ...
                if not register_response.ok or not register_response.text.lower() == "success":
                    self.ip = ""
                    sleep(5)
    ...
    def heartbeat(self):
        if self.ip:
            try:
                ...
                if register_response.ok and register_response.text.lower() == "success":
                    self.failed_heartbeat_count = 0
                    if register_response.text.lower() != "success":
                        self.ip = ""
                else:
...
def main(args):
    ...
    load_bal_manager = LoadBalManager(server_type, authed_session)
